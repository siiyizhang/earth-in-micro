...
- Life Tree reveal (`LifeTreeReveal.tsx`, port of iOS `LifeTreeReveal`): after an upload with identified families, or an edit that changes a discovery's family, the whole tree is shown as a dim radial disc with the account's earlier branches lit and the new branches growing from the centre (2.6 s), family names fading in; Save downloads the PNG, Share uses the Web Share API where available, and Open Life Tree links to `/app/gallery?view=tree`. Triggered through `revealStore.ts`; hosted once in `WebApp`.
- Onboarding tour (`OnboardingTour.tsx`): shown once per browser on Explore (`localStorage` key `micro-tour-done`; replay from Settings). Welcome card, then spotlights on Create, a sample identification, the Life Tree reveal in demo mode, the place cards with a sample new card sliding in, and Gallery. Spotlight targets are `data-tour` attributes.

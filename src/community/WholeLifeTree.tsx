...
import { lifemapLayout, radialEdge, radialLayout, ringRadius } from "./lifeTreeLayout";
import type { Layout, LifeTreeStyle, Point } from "./lifeTreeLayout";
...
function paint(ctx: CanvasRenderingContext2D, width: number, height: number, dpr: number, layout: Layout, nodes: LifeTreeNode[], view: View, fit: number, selectedId: string | null, restrictToLit: boolean) {
  ...
  for (const n of nodes) {
    ...
    let c1 = a, c2 = b;
    if (layout.style === "radial") {
      const [, f, t] = radialEdge(layout.origin, from, to);
      c1 = { x: sx(f), y: sy(f) };
      c2 = { x: sx(t), y: sy(t) };
    }
